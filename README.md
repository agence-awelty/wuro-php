# Wuro PHP API library

> [!NOTE]
> The Wuro PHP API Library is currently in **beta** and we're excited for you to experiment with it!
>
...
$client = new Client(
  appID: getenv('WURO_APP_ID') ?: 'My App ID',
  appSecret: getenv('WURO_APP_SECRET') ?: 'My App Secret',
);

$invoice = $client->invoices->create();

var_dump($invoice->newInvoice);
```
...
When the library is unable to connect to the API, or if the API returns a non-success status code (i.e., 4xx or 5xx response), a subclass of `Wuro\Core\Exceptions\APIException` will be thrown:
